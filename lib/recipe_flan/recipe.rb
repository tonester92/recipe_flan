...
attr_accessor :name, :ingredients, :directions, :reviews, :url
  ...
  def self.scrape_foodnetwork
    ...
    recipe = self.new  
    recipe.name = doc.search("h1.o-AssetTitle__a-Headline").text.gsub("\n","").strip
    recipe.ingredients = doc.search("div.o-Ingredients__m-Body").text.gsub("\n","").strip
    recipe.directions = doc.search("div.o-Method__m-Step").text.gsub("\n","").strip
  ...
  def self.scrape_allrecipes
    ...
    recipe = self.new  
    recipe.name = doc.search("h1.recipe-summary__h1").text.gsub("\n","").strip
    recipe.ingredients = doc.search("li.checkList__line").text.gsub("\n","").strip
    recipe.directions = doc.search("span.recipe-directions__list--item").text.gsub("\n","").strip
    recipe.reviews = doc.search("span.review-count").text.gsub("\n","").strip
